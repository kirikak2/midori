...
### キャッシュの書き戻し（2026-08-10）

Tab5 では**描いただけでは画面に出ない**。`Panel_FrameBufferBase` は変更範囲を
溜めて `display()` で書き戻すが、`endWrite()` はそれを呼ばない。呼ばずにいると
キャッシュラインが偶然追い出されたときにしか PSRAM のフレームバッファへ届かず、
新旧の絵が混ざる。

この画面の描画は**細い円弧と数文字の更新**という、まさに追い出されにくい
散らばった小領域なので、これが最初に露呈したのが Knobs 画面だった
（大きな塗りつぶしは互いを追い出すので目立たない）。

`UIManager::update()` の最後で `M5.Lcd.display()` を 1 回呼ぶことで解決している。
変更範囲だけを書き戻すので、値が 1 つ動いただけのフレームは安い。
副次的に、セルを黒で消してから描き直す全面再描画も、途中経過が PSRAM へ
出ないぶんちらつきにくくなる。
